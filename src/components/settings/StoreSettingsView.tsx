...
type SettingsTab = 'rbac_staff' | 'fraud_security' | 'bank_payout' | 'store_profile' | 'audio_alerts';
...
export const StoreSettingsView: React.FC = () => {
  ...
  const [activeSoundPreview, setActiveSoundPreview] = useState<SoundKey | null>(null);

  const handlePlaySound = (key: SoundKey) => {
    setActiveSoundPreview(key);
    soundService.play(key);
    setTimeout(() => {
      setActiveSoundPreview(prev => (prev === key ? null : prev));
    }, 650);
  };
  ...
  const handlePlayAllTones = async () => {
    for (let i = 0; i < STATUS_SOUND_CATALOG.length; i++) {
      ...
      setActiveSoundPreview(item.key);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
        {[
          ...
          { id: 'audio_alerts', label: 'Mandatory Audio Chimes', icon: Volume2 },
        ].map(tab => {
          const Icon = tab.icon;
      ...
      {/* Tab 5: Mandatory Audio Chimes & Sound Synthesis Engine */}
      {activeTab === 'audio_alerts' && (
        <div className="space-y-4">
          {/* Mandatory Policy Card */}
          <div className="bg-amber-50/80 border border-amber-200/90 rounded-2xl p-4 sm:p-5 flex items-start gap-3.5 text-amber-950">
            ...
            <div className="space-y-1">
              <h3 className="text-sm font-bold text-amber-900">Audio Alerts Are Strictly Mandatory</h3>
              <p className="text-xs text-amber-800 leading-relaxed max-w-3xl">
                QCOM guarantees a 10-minute turnaround for customer orders and enforces a strict 3-minute in-store picking SLA. 
                Disabling or muting audio feedback is strictly prohibited on fulfillment terminals. 
                Each order status has an acoustically distinct chime so store associates can immediately recognize order lifecycle transitions 
                without continuously monitoring the workstation screen.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 p-5 shadow-2xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
              <div>
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  ...
                  Status Audio Signatures ({STATUS_SOUND_CATALOG.length} Distinct Tones)
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  High-fidelity synthesized chimes produced client-side with the Web Audio API (zero audio file fetch latency).
                </p>
              </div>

              <button
                type="button"
                onClick={handlePlayAllTones}
                className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs transition-colors cursor-pointer self-start sm:self-auto shadow-2xs"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {STATUS_SOUND_CATALOG.map(sound => {
                const isPlaying = activeSoundPreview === sound.key;
                return (
                  <div
                    key={sound.key}
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <span className="font-bold text-slate-900 text-xs">{sound.title}</span>

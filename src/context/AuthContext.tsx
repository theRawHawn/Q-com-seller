...
interface AuthContextType {
  ...
  updateStaffCustomPermissions: (staffId: string, customPermissions: PermissionKey[] | null) => boolean;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateStaffCustomPermissions = (staffId: string, customPermissions: PermissionKey[] | null): boolean => {
    if (currentUser.role !== 'STORE_OWNER') {
      showToast('Permission Denied', 'Only the Primary Store Owner can customize individual staff access.', 'error');
      return false;
    }
    const target = staffMembers.find(m => m.id === staffId);
    setStaffMembers(prev => {
      const updated = prev.map(m => (m.id === staffId ? { ...m, customPermissions: customPermissions || undefined } : m));
      localStorage.setItem('qcom_store_staff_members', JSON.stringify(updated));
      return updated;
    });
    showToast(
      'Custom Access Updated',
      customPermissions
        ? `Custom component overrides applied for ${target?.name || 'staff member'}.`
        : `Reset ${target?.name || 'staff member'} to default role permissions.`,
      'success'
    );
    return true;
  };
  ...
  const addStaffMember = (member: Omit<StoreStaffMember, 'id' | 'joinedDate' | 'lastActive'>): boolean => {
    ...
    showToast(
      'Onboarding Notification Dispatched',
      `Invitation email sent to ${member.email}. Staff will create password for daily login.`,
      'success'
    );
    return true;
  };
  ...
  const hasPermission = useCallback(
    (permission: PermissionKey): boolean => {
      ...
      const currentStaff = staffMembers.find(m => m.id === currentUser.id);
      if (currentStaff?.customPermissions && Array.isArray(currentStaff.customPermissions)) {
        return currentStaff.customPermissions.includes(permission);
      }
      const permissions = rolePermissions[currentUser.role] || [];
      return permissions.includes(permission);
    },
    [currentUser.id, currentUser.role, rolePermissions, staffMembers]
  );
